...
.bookingCalendarCount{display:none!important}
...
function removeLegacyCounts(){document.querySelectorAll('.bookingCalendarCount').forEach(x=>x.remove())}

function ensureCalendar(){
  const date=$('date');
  if(!date||$('bookingAvailabilityCalendar')){removeLegacyCounts();return;}
...
function bindMenuButtons(){
  ...
  buttons.forEach((b,i)=>{
    ...
    if(b.dataset.calendarBoundV2==='1')return;
    b.dataset.calendarBoundV2='1';
...
function setupLegacyCountRemoval(){
  removeLegacyCounts();
  new MutationObserver(removeLegacyCounts).observe(document.body,{childList:true,subtree:true});
}
